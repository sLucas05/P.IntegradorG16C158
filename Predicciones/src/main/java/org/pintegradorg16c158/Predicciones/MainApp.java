...
public class MainApp {
	public static void main(String[] args) {
		if (args.length < 2) {
			System.out.println(
					"\nDebe ingresar la ruta de los archivos, primero Partidos.csv y luego Prediccion.csv\n(los nombres pueden ser modificados pero no la extension .csv)");
			return;
		}
		...
		try {
			...
			CSVwrite.print(equipo1 + Separador + partido1.getGolesEquipo1() + Separador + partido1.getGolesEquipo2()
					+ Separador + equipo2 + "\n");
			CSVwrite.print(equipo3 + Separador + partido2.getGolesEquipo1() + Separador + partido2.getGolesEquipo2()
					+ Separador + equipo4 + "\n");
		} catch (IOException x) {
			x.printStackTrace();
		} finally {
				...
				for (String[] fila : filas) {
					...
					if (contador == 0) {
						...
						if (golesA > golesB) {
							...
							System.out.println(
									"\n[" + golesA + "]" + equipo1 + " - " + equipo2 + "[" + golesB + "] // Empate");
